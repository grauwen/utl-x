...
Three things fix its place in the method. First, it is *preserved* where it counts. *Schema* documentation — describing the class — is lifted into the model as the schema is parsed and normalised across formats into one representation (USDL's `%documentation` and `%description`), so the matcher need never know which format a schema came from. An *instance* can carry documentation too — a comment beside a value, a stray annotation — and it is not worthless, but it is a weaker and different thing: instance evidence, about one document rather than the class, to which Chapter 3's caution applies. It may serve as a sample-grade hint or tie-breaker, never as a claim about the contract — the more so because such notes often describe the *occurrence* ("this order is a rush") rather than the field's meaning, and only the latter helps a matcher. And unlike schema documentation, instance comments are discarded at parse time today; using them at all would mean preserving them in the model's metadata side-channel rather than dropping them.
