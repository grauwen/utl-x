...
There is a companion to this volume, _UTLX: One Language, All Formats_, that teaches you how to write transformations. This book is about something quieter and, in the long run, more important: how to *reason* about them.
...
The good news is that the theory already exists, scattered across four decades of database research: schema matching, schema mapping, data exchange, data provenance. None of it was written with UTLX in mind. All of it applies. This book gathers that material, translates it out of its native dialect, and lands it on UTLX's two concrete foundations — the *Universal Data Model* and the distinction between *Execution mode* and *Message Contract mode*.

== What This Book Is, and Is Not

This is a *theory* book. It contains very little UTLX syntax. Where the companion volume shows you `concat`, `map`, and `groupBy`, this book shows you *why* a given output field demands an aggregation, *why* a second input is a lookup rather than a co-equal source, and *why* the mapping you wrote is — or is not — sound.
...
*Part III — From Theory to Transformation* turns the theory into a working pipeline: typing the inputs, analysing the outputs, inferring functions, and generating idiomatic UTLX strategy-first.

A note on cross-references: throughout, design decisions are tagged with their origin in the UTLX feature record (IF11 coverage, IF20 strategy, IF21 the visual mapper). You do not need those documents to read the book; the tags simply mark where theory met implementation.
