...
Pre-validation prevents wasted transformation work on bad input. Post-validation catches mapping bugs before the output leaves. This is the pattern that UTLXe's `ValidationOrchestrator` implements (Chapter 19).
