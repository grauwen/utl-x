...
All five patterns below are shown with the same domain — an Order containing a Customer containing an Address — so you can focus on *how each pattern organizes the same structure* (global vs. local elements, named vs. anonymous types) rather than on changing example data. The final pattern (Swiss Army Knife) adds a few extra fields to show a realistic mixed schema.
