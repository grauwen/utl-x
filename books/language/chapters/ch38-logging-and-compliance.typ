...
UTL-X's security library (Chapter 17) provides functions for masking, hashing, and encrypting sensitive data *within* transformations. If you need to produce an audit-safe version of the output:
...
This produces a record that's safe to log, store in a data warehouse, or send to analytics — the sensitive fields are masked or hashed, the business metadata is preserved. See Chapter 17 for the full security function reference.
