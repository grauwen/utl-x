...
SNS fans out to multiple SQS queues, each with its own UTLXe container running a different transformation. One event, multiple output formats — the fan-out pattern from Chapter 20.
...
Since UTLXe is a JVM application, JMS integration uses the Java SDK (Chapter 34) directly — no Dapr, no HTTP, no subprocess. The transformation engine runs in-process inside your JMS consumer:
...
For organizations that prefer decoupled deployment, the Java stdio wrapper (Chapter 34) works too — your JMS consumer spawns UTLXe as a subprocess and communicates via protobuf. This allows upgrading UTLXe independently from the JMS application.
...
Pre-validation (Chapter 19) catches malformed input before the transformation runs:
