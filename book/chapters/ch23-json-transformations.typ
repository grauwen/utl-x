...
The key insight: CSV is flat, so you must denormalize — repeat parent fields (orderId, customer) for each child row. For complex hierarchies, the `unnest()` function (Chapter 20) automates this.
...
Converts an XSD schema to JSON Schema 2020-12 via the USDL intermediate representation. The USDL tier system (Chapter 11) determines which constraints survive the conversion.
