...
XML canonicalization (W3C C14N). See Chapter 21 for full details.
...
Create a structured QName from its parts. See Chapter 21.
...
Filter CSV rows by a column condition. Returns a new CSV string. See Chapter 24.
...
Convert between objects and `[key, value]` pair arrays. Essential for dynamic key processing. See Chapter 25.
...
Get all namespace declarations from an XML element as a prefix-to-URI map. See Chapter 21.
...
JSON Canonicalization Scheme (RFC 8785). Produces deterministic JSON — identical output regardless of key order or whitespace. See Chapter 23.
...
QName decomposition — extract parts of an XML qualified name. See Chapter 21 for full details and XBRL examples.
...
Transform object keys and/or values. See Chapter 25 (dynamic keys).
...
Mask a string, keeping only the first N characters visible. For PII protection in logs and reports. See Chapter 37.
...
Also: `matchesQname(element, pattern)` for XML QName matching (Chapter 21).
...
Transform object keys and/or values. See Chapter 25.
...
Parse regional number formats to standard floating-point. See Chapter 24 (CSV).
...
YAML-specific functions for multi-document handling and path-based access. See Chapter 25.
