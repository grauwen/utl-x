...
#table(
  ...
  [Pipeline], [Pipeline chaining (Chapter 19)],
  [Transform stage], [`.utlx` transformation file],
  [Validate stage], [Schema validation (Chapter 18)],
...
Open-M is written in Go — lightweight, fast startup, single binary, native Kubernetes integration. It communicates with UTLXe via the Go wrapper (Chapter 33) using protobuf over stdio.
...
The Go wrapper bridges them — the controller spawns UTLXe as a subprocess and communicates via the uniform SDK interface (Chapter 33).
