...
Function names MUST start with an uppercase letter (PascalCase). See Chapter 14.
...
USDL is a dialect within UTL-X — not a separate language (see Chapter 11). When `%usdl 1.0` is declared on the output, the data is structured using USDL directives. These directives use the `%` prefix and follow a fixed grammar.
...
This USDL document can be converted to JSON Schema, XSD, Avro, Protobuf, or EDMX — the USDL tier system determines what survives in each target format (Chapter 11).
