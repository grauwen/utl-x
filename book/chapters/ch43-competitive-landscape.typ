...
*UTL-X:* all formats, modern functional syntax, Chapter 35 provides a migration guide. XSLT is still dominant in government, banking, and healthcare legacy — but new projects increasingly choose alternatives.
...
*UTL-X:* open source (AGPL), \$35/month on Azure Marketplace, `.utlx` files version-controlled in Git, no vendor lock-in. Chapter 35 covers DataWeave migration.
...
*Limitations:* XML-centric (JSON requires conversion steps), expensive licensing (\$50K+/year), heavy runtime, Russian Doll XSD patterns (Chapter 29).

*UTL-X:* native JSON/CSV/YAML, lightweight container vs full application server, 10-100x cheaper. Chapter 35 covers BW migration, Chapter 29 covers XSD pattern conversion.
...
*UTL-X advantage:* declarative transformation language vs imperative Java code. Camel does routing and connectivity; UTL-X does transformation. They can work together — Camel routes messages, UTL-X transforms them (via the Java SDK, Chapter 34).
...
*UTL-X advantage:* simpler syntax, cloud-native (container), active development. When UTL-X adds EDI support (Chapter 42), Smooks' main differentiator disappears.
...
+ *AI-friendly:* natural language round-trip, MCP server (Chapter 45)
