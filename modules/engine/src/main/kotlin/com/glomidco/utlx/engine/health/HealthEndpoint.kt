package com.glomidco.utlx.engine.health
...
import com.glomidco.utlx.engine.EngineState
        ...
        for (attempt in 0..maxRetries) {
            ...
            try {
                ...
                server = embeddedServer(Netty, port = tryPort, host = "0.0.0.0") {
                    ...
                    com.glomidco.utlx.engine.admin.configureAdmin(this, engine, adminKey, dataDir, engine.daprIntegration)
                }.start(wait = false)
...
fun Application.configureHealth(engine: UtlxEngine) {
    ...
    routing {
        ...
        get("/health/ready") {
            val txCount = engine.registry.list().size
            val isReady = engine.state == EngineState.RUNNING && txCount > 0
            if (isReady) {
                call.respond(HttpStatusCode.OK, mapOf(
                    "status" to "UP",
                    "state" to engine.state.name,
                    "transformations" to txCount,
                    "ready" to true
                ))
            } else {
