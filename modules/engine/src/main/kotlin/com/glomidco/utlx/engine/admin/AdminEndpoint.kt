...
import com.glomidco.utlx.engine.registry.TransformationRegistry
import com.glomidco.utlx.engine.transport.TransportHandlers
import com.glomidco.utlx.engine.registry.TransformationInstance
import com.glomidco.utlx.engine.strategy.HeaderSchemaInfo
import com.glomidco.utlx.engine.util.UuidV7
import com.glomidco.utlx.engine.validation.SchemaValidatorFactory
    ...
    app.routing {
        ...
        route("/admin") {
            ...
            post("/bundle") {
                ...
                val configMap = mutableMapOf<String, com.glomidco.utlx.engine.config.TransformConfig>()
                                ...
                                if (parts.size == 2) {
                                    ...
                                    when {
                                        ...
                                        path.endsWith("transform.yaml") -> {
                                            ...
                                            try {
                                                val yamlMapper = com.glomidco.utlx.engine.config.TransformConfig.yamlMapper()
                                                configMap[name] = yamlMapper.readValue(yamlContent, com.glomidco.utlx.engine.config.TransformConfig::class.java)
                                            } catch (e: Exception) {
                                ...
                                if (parts.size == 2) {
                                    ...
                                    try {
                                        val strategy = engine.createStrategy(
                                            com.glomidco.utlx.engine.config.TransformConfig(strategy = "COMPILED")
                                        )
                                        strategy.initialize(source, com.glomidco.utlx.engine.config.TransformConfig())
                                        strategy.shutdown()
                                        found++
                                    } catch (e: Exception) {
            ...
            post("/transformations/{name}/config") {
                ...
                val newInputs = if (tree.has("inputs")) {
                    tree.get("inputs").map { node ->
                        com.glomidco.utlx.engine.config.InputSlot(
                ...
                val updatedConfig = tx.config.copy(
                    ...
                    inputs = newInputs,
                    output = com.glomidco.utlx.engine.config.OutputSlot(schema = newOutputSchema)
                )
            ...
            get("/info") {
                call.respond(HttpStatusCode.OK, mapOf<String, Any?>(
                    ...
                    "ready" to (engine.state == com.glomidco.utlx.engine.EngineState.RUNNING && registry.list().isNotEmpty()),
                    ...
                    "tracing" to com.glomidco.utlx.engine.telemetry.Tracing.isActive,
                    "tracing_agent" to if (com.glomidco.utlx.engine.telemetry.Tracing.isActive) "Azure Monitor agent loaded" else "disabled (set APPLICATIONINSIGHTS_CONNECTION_STRING to enable)",
...
private fun parseMessagingEndpoint(node: com.fasterxml.jackson.databind.JsonNode?): com.glomidco.utlx.engine.config.MessagingEndpoint? {
    ...
    return com.glomidco.utlx.engine.config.MessagingEndpoint(
        queue = queue, topic = topic, eventhub = eventhub,
        subscription = subscription, consumerGroup = consumerGroup
    )
}

private fun endpointToMap(ep: com.glomidco.utlx.engine.config.MessagingEndpoint, daprStatus: String): Map<String, Any?> {
...
private fun buildMessagingResponse(
    config: com.glomidco.utlx.engine.config.TransformConfig,
    syncState: SyncState
): Map<String, Any?>? {
...
private fun resolveSchemaValidators(
    ...
    if (schemasWithRefs.size <= 1 && schemaInfo.inputSchemaRef != null) {
        ...
        val multiValidators = mutableMapOf<String, com.glomidco.utlx.engine.validation.SchemaValidator>()
...
private fun createValidatorFromStore(
    ...
    format: String,
    schemaStore: SchemaStore
): com.glomidco.utlx.engine.validation.SchemaValidator? {
