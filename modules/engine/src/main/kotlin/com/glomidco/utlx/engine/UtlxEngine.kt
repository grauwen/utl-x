...
import com.glomidco.utlx.engine.bundle.BundleLoader
import com.glomidco.utlx.engine.config.EngineConfig
...
import com.glomidco.utlx.engine.strategy.CompiledStrategy
import com.glomidco.utlx.engine.strategy.CopyStrategy
import com.glomidco.utlx.engine.strategy.ExecutionStrategy
import com.glomidco.utlx.engine.strategy.TemplateStrategy
import com.glomidco.utlx.engine.transport.TransportServer
...
class UtlxEngine(val config: EngineConfig) {
    ...
    val validationOverrides = com.glomidco.utlx.engine.admin.ValidationOverrideStore()
    var daprIntegration: com.glomidco.utlx.engine.admin.DaprIntegration? = null
    var bundleInfo: com.glomidco.utlx.engine.admin.BundleInfo = com.glomidco.utlx.engine.admin.BundleInfo(mode = "open")
                    ...
                    if (java.nio.file.Files.exists(utlxFile)) {
                        try {
                            ...
                            val transformConfig = if (java.nio.file.Files.exists(configFile)) {
                                try {
                                    com.glomidco.utlx.engine.config.TransformConfig.load(configFile)
                                } catch (e: Exception) {
                                    logger.warn("Failed to load transform.yaml for '{}': {}, using defaults", name, e.message)
                                    com.glomidco.utlx.engine.config.TransformConfig()
                                }
                            } else {
                                com.glomidco.utlx.engine.config.TransformConfig()
                            }
                            val strategy = createStrategy(
                                com.glomidco.utlx.engine.config.TransformConfig(strategy = transformConfig.strategy.ifEmpty { "COMPILED" })
                            )
                            strategy.initialize(source, transformConfig)
                            val instance = com.glomidco.utlx.engine.registry.TransformationInstance(
                                name = name,
                                source = source,
                                strategy = strategy,
                                config = transformConfig
    ...
    private fun resolveValidatorFromConfig(
        ...
        try {
            ...
            return com.glomidco.utlx.engine.validation.SchemaValidatorFactory.create(content, format)
        } catch (e: Exception) {
