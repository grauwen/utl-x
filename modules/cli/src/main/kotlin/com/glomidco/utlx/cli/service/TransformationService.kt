// modules/cli/src/main/kotlin/com/glomidco/utlx/cli/service/TransformationService.kt
package com.glomidco.utlx.cli.service

import com.glomidco.utlx.cli.commands.TransformCommand
...
import com.glomidco.utlx.formats.csv.RegionalFormat
import com.glomidco.utlx.formats.jsch.JSONSchemaParser
import com.glomidco.utlx.formats.jsch.JSONSchemaSerializer
import com.glomidco.utlx.formats.json.JSONParser
...
import com.glomidco.utlx.formats.tsch.TableSchemaParser
import com.glomidco.utlx.formats.tsch.TableSchemaSerializer
...
class TransformationService {
        ...
        private fun ensureStdlibLookup() {
            if (stdlibLookup == null) {
                synchronized(this) {
                    if (stdlibLookup == null) {
                        try {
                            val functions = com.glomidco.utlx.stdlib.StandardLibrary.getAllFunctions()
    ...
    fun serializeOutputPublic(
        udm: UDM,
        format: String,
        formatSpec: com.glomidco.utlx.core.ast.FormatSpec,
        pretty: Boolean = true
    ): String {
            ...
            when (format.lowercase()) {
                "auto" -> {
                    ...
                    val detectedFormat = when {
                        ...
                        // Detect CSV: look for comma-separated values with consistent column counts
                        trimmed.contains(",") && trimmed.lines().size > 1 -> {
                            val lines = trimmed.lines().filter { it.isNotBlank() }
                            val firstLineCommas = lines.firstOrNull()?.count { it == ',' } ?: 0
                            // If multiple lines have similar comma counts, likely CSV
     ...
     */
    private fun serializeOutput(udm: UDM, format: String, formatSpec: com.glomidco.utlx.core.ast.FormatSpec, pretty: Boolean): String {
        return try {
            when (format.lowercase()) {
                "xml" -> {
     ...
     */
    fun serializeOutputToBytes(udm: UDM, format: String, formatSpec: com.glomidco.utlx.core.ast.FormatSpec, pretty: Boolean): ByteArray {
        return when (format.lowercase()) {
            "xml" -> {
                val encoding = formatSpec.options["encoding"] as? String
