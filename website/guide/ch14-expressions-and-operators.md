...
UTL-X is an expression-based language — everything produces a value.
There are no statements, no void functions, no side effects. A
transformation body is one expression (possibly with `let` bindings)
that evaluates to the output.
...
Division by zero throws a runtime error. Use a guard if the divisor
might be zero:
...
Short-circuit evaluation: `&&` stops at the first `false`, `||` stops at
the first `true`. This is useful for guarding expressions:
...
For complex string building, chain multiple `concat()` calls or use
`join()`:
...
Objects are created with curly braces. This is both the most common
expression and the most powerful:
...
Include properties only when a condition is true by spreading a
conditional object:
...
Arrays are created with square brackets and processed with higher-order
functions:
...
argument to the right function. It makes data flow readable — left to
right instead of nested inside-out:
...
Both produce the same result: a sorted list of unique categories. But
the pipe version reads like a recipe: "take items, map to categories,
...
Evaluates left to right — the first non-null value wins. Essential for
...
These patterns compose naturally — every sub-expression produces a
value, and values flow through operators and functions without
intermediate variables (unless you want readability via `let`).
