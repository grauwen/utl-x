...
This chapter presents real-world integration scenarios and reusable
transformation recipes. Each case study shows the architecture, the
transformation code, and the results. The recipes are standalone
patterns you can copy and adapt.

## Case Study 1: European E-Invoicing (Peppol / UBL)

**Scenario:** A Dutch wholesale distributor (200 employees, 850 B2B
customers) must comply with EU Directive 2014/55 — all B2G invoices in
UBL 2.1 XML via the Peppol network.

**Source:** Dynamics 365 Business Central (OData JSON). **Target:** UBL
2.1 XML, delivered via Peppol BIS 3.0 Access Point.

**Architecture:**

![](/pictures/ch47-cases-studies/usecase-1.png)

Use Case 1 — Dynamics 365 → Service Bus → UTL-X Container App → Peppol
Access Point. Three hot-reloadable transformations handle invoice
mapping, validation, and country-specific routing.

![](/pictures/ch47-cases-studies/usecase-t-1.png)

Use Case 1 — The UTL-X transformation in detail: Dynamics 365 JSON input
(left), invoice-to-ubl.utlx transformation (center), UBL 2.1 XML output
(right).
...
![](/pictures/ch47-cases-studies/usecase-b-1.png)

Use Case 1 — Business impact: before vs after UTL-X, with production
...
- Savings: €44,000/year vs previous solution (12 custom Azure Functions
  in C#)

## Case Study 2: Healthcare — FHIR Integration

**Scenario:** A Dutch academic hospital (1,200 beds) must exchange
patient data via FHIR R4 for the MedMij/VIPP national health information
exchange program.

**Sources:** Lab system (HL7 v2 → JSON via Mirth), Radiology (DICOM SR →
JSON), EHR (proprietary JSON), Pharmacy (HL7 v2.5 → JSON). **Target:**
FHIR R4 Bundles via the NUTS network.

**Architecture:**

![](/pictures/ch47-cases-studies/usecase-2.png)

Use Case 2 — Four source systems (Lab, Radiology, EHR, Pharmacy) →
Service Bus → UTL-X Container App → FHIR Server → NUTS Network. All
processing stays inside the hospital's Azure tenant for GDPR + NEN 7510
compliance.

![](/pictures/ch47-cases-studies/usecase-t-2.png)

Use Case 2 — Transformation detail: HL7 v2.3 OBX message parsed to JSON
(left), hl7v2-to-fhir.utlx transformation excerpt (center), FHIR R4
Observation Bundle (right).
...
![](/pictures/ch47-cases-studies/usecase-b-2.png)

Use Case 2 — Business impact: before vs after UTL-X, with production
...
- 100% NEN 7510 compliant by architecture (all data stays in hospital's
  Azure tenant)
...
**Scenario:** An online retailer with 4 sales channels needs a unified
order format for their ERP, warehouse, and accounting systems.

**Sources:** Shopify (JSON), WooCommerce (JSON), Amazon (XML), manual
entry (CSV). **Target:** canonical order JSON → fan-out to ERP (JSON),
warehouse (XML), accounting (CSV).
...
Four normalizers (one per channel) produce the same canonical format.
Three output formatters transform the canonical format to ERP,
warehouse, and accounting. Total: 7 `.utlx` files.
...
Different systems format addresses differently. Normalize to a common
structure:
...
Different APIs return errors in different formats. Normalize to one
structure:
...
Convert US CSV (comma decimal) to European CSV (comma delimiter
conflicts — use semicolons):
...
One line of transformation body. The format options handle the rest —
comma decimals, dot thousands, semicolon delimiters.

### Recipe 7: XML Namespace Stripping

Remove all namespace prefixes from XML for downstream systems that don't
handle them:
...
The transformation accesses elements by their prefixed names (bracket
notation) but outputs clean, unprefixed property names.
...
The `format` field in the input controls whether the output is a summary
or full detail.
