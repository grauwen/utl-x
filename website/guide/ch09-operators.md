...
Division always produces a floating-point result — `10 / 3` is
`3.333...`, not `3`. There is no integer division operator. Use
`floor(10 / 3)` if you need integer results.

Division by zero throws a runtime error. Guard if the divisor might be
zero:
...
stops at the first `true`. This is useful for guarding expressions:
...
In boolean contexts, UTL-X treats these as *falsy*:
...
Everything else is *truthy*, including empty arrays `[]` and empty
objects `{}`.
...
Two operators for dealing with null values — the most common source of
runtime errors in data transformation:
...
Without `?.`, a null intermediate value causes a runtime error. With
...
These operators navigate the UDM tree — accessing properties,
attributes, metadata, and children:
...
The `.` navigates data (what the user sent). The `@` navigates XML
attributes (format-specific metadata on elements). The `^` navigates
internal metadata (parser-generated, never in output). See Chapter 8 for
...
Spread is shallow — nested objects are referenced, not deep-copied. See
Chapter 8 for the full spread operator guide.
...
The brackets evaluate the expression and use the result as the key. See
Chapter 26 (dynamic keys) for advanced patterns.

## Flow Operators

### Pipe (`|>`)

Pass the result of the left expression as the first argument to the
right function. Reads left-to-right instead of nested inside-out:
...
Both produce the same result. The pipe version reads like a recipe:
"take items, map to categories, remove duplicates, sort."
...
The `->` is the convention used throughout this book. The `=>` is
accepted for developers coming from JavaScript/TypeScript. The `=>` also
appears in match expression arms:
...
The `=` is NOT assignment (there are no mutable variables in UTL-X). It
is a one-time binding — the name refers to the value for the rest of its
scope. See Chapter 8 for let binding scoping rules.

## String Concatenation

UTL-X does NOT use `+` for string concatenation — use the `concat()`
function:
...
This is deliberate. `+` is arithmetic only. Mixing `+` for both numbers
and strings leads to ambiguity (`"3" + 4` — is it `"34"` or `7`?). UTL-X
avoids this by keeping arithmetic and string operations separate.
...
When in doubt, use parentheses. They make intent explicit and prevent
