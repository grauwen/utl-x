...
class XMLAttributeSyntaxTest {
    ...
    private fun executeTransformation(transformation: String, input: UDM): UDM {
        val tokens = com.glomidco.utlx.core.lexer.Lexer(transformation).tokenize()
        val parseResult = com.glomidco.utlx.core.parser.Parser(tokens).parse()

        if (parseResult is com.glomidco.utlx.core.parser.ParseResult.Failure) {
            val errors = parseResult.errors.joinToString("\n") { "  - $it" }
            throw RuntimeException("Parse failed:\n$errors")
        }

        val program = (parseResult as com.glomidco.utlx.core.parser.ParseResult.Success).program
        val runtimeValue = com.glomidco.utlx.core.interpreter.Interpreter().execute(program, input)
    ...
    fun `parse and accept @attribute syntax for single attribute output`() {
        ...
        val input = com.glomidco.utlx.formats.json.JSON.parse("""{"customer": "Alice"}""")
    ...
    fun `parse and accept @attribute syntax for multiple attributes`() {
        ...
        val input = com.glomidco.utlx.formats.json.JSON.parse("""{}""")
        val result = executeTransformation(transformation, input)
        val xml = XMLSerializer(prettyPrint = false, includeDeclaration = false).serialize(result)
    ...
    fun `parse and accept @attribute syntax with expression values`() {
        ...
        val input = com.glomidco.utlx.formats.json.JSON.parse("""{"year": "2025", "id": "123"}""")
        val result = executeTransformation(transformation, input)
        val xml = XMLSerializer(prettyPrint = false, includeDeclaration = false).serialize(result)
    ...
    fun `read XML attributes from input using @attribute syntax`() {
        ...
        val result = executeTransformation(transformation, input)
        val json = com.glomidco.utlx.formats.json.JSON.stringify(result)
    ...
    fun `handle attribute values with special characters requiring escaping`() {
        ...
        val input = com.glomidco.utlx.formats.json.JSON.parse("""{"title": "Smith & Co", "note": "A < B"}""")
        val result = executeTransformation(transformation, input)
        val xml = XMLSerializer(prettyPrint = false, includeDeclaration = false).serialize(result)
    ...
    fun `handle numeric and boolean attribute values`() {
        ...
        val input = com.glomidco.utlx.formats.json.JSON.parse("""{"quantity": 42, "price": 29.99, "inStock": true}""")
        val result = executeTransformation(transformation, input)
        val xml = XMLSerializer(prettyPrint = false, includeDeclaration = false).serialize(result)
