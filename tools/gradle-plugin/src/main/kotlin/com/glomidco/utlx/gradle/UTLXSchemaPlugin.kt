// tools/gradle-plugin/src/main/kotlin/com/glomidco/utlx/gradle/UTLXSchemaPlugin.kt
package com.glomidco.utlx.gradle
...
abstract class GenerateSchemaTask : DefaultTask() {
    ...
    private fun parseInputSchema(content: String, format: String): com.glomidco.utlx.analysis.types.TypeDefinition {
        return when (format.lowercase()) {
            "xsd" -> {
                val parser = com.glomidco.utlx.analysis.schema.XSDSchemaParser()
                parser.parse(content, com.glomidco.utlx.analysis.schema.SchemaFormat.XSD)
            }
            "json-schema" -> {
                val parser = com.glomidco.utlx.analysis.schema.JSONSchemaParser()
                parser.parse(content, com.glomidco.utlx.analysis.schema.SchemaFormat.JSON_SCHEMA)
            }
            else -> throw IllegalArgumentException("Unsupported input format: $format")
        }
    }
    
    private fun parseTransformation(content: String): com.glomidco.utlx.core.ast.Program {
        val parser = com.glomidco.utlx.core.parser.Parser()
        return parser.parse(content)
    }
    
    private fun inferOutputType(
        program: com.glomidco.utlx.core.ast.Program,
        inputType: com.glomidco.utlx.analysis.types.TypeDefinition
    ): com.glomidco.utlx.analysis.types.TypeDefinition {
        val inference = com.glomidco.utlx.analysis.types.AdvancedTypeInference(inputType)
        return inference.inferOutputType(program)
    }
    
    private fun generateSchema(
        type: com.glomidco.utlx.analysis.types.TypeDefinition,
        format: String,
        pretty: Boolean,
        ...
        return when (format.lowercase()) {
            "json-schema" -> {
                val generator = com.glomidco.utlx.analysis.schema.JSONSchemaGenerator()
                generator.generate(
                    type,
                    com.glomidco.utlx.analysis.schema.SchemaFormat.JSON_SCHEMA,
                    com.glomidco.utlx.analysis.schema.GeneratorOptions(
                        pretty = pretty,
...
abstract class ValidateTransformTask : DefaultTask() {
    ...
    fun validate() {
        ...
        try {
            ...
            // Validate
            val validator = com.glomidco.utlx.analysis.schema.TransformValidator()
    ...
    private fun parseInputSchema(content: String): com.glomidco.utlx.analysis.types.TypeDefinition {
        // Detect and parse input schema
        val parser = com.glomidco.utlx.analysis.schema.XSDSchemaParser()
        return parser.parse(content, com.glomidco.utlx.analysis.schema.SchemaFormat.XSD)
    }
    
    private fun parseOutputSchema(content: String): com.glomidco.utlx.analysis.types.TypeDefinition {
        // Detect and parse output schema
        val parser = com.glomidco.utlx.analysis.schema.JSONSchemaParser()
        return parser.parse(content, com.glomidco.utlx.analysis.schema.SchemaFormat.JSON_SCHEMA)
    }
