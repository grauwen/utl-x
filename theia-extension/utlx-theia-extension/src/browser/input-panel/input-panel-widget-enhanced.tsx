/**
 * Enhanced Input Panel Widget
 *
 * Updated version with file system integration
 */
...
@injectable()
export class InputPanelWidgetEnhanced extends ReactWidget {
    ...
    @inject(UTLXFileService)
    protected readonly fileService!: UTLXFileService;
    ...
    constructor() {
        super();
        this.id = InputPanelWidgetEnhanced.ID;
        this.title.label = InputPanelWidgetEnhanced.LABEL;
    ...
    private async handleLoadFile(): Promise<void> {
        try {
            ...
            const fileContent = await this.fileService.openFile({
                title: 'Load Input File',
                canSelectMany: false
            });

            if (fileContent) {
                const detectedFormat = this.fileService.detectFormat(fileContent.uri);
                ...
                this.messageService.info(`Loaded: ${fileContent.name} (${detectedFormat})`);
            } else {
