...
Native binaries (no JVM required) are available from [GitHub Releases](https://github.com/grauwen/utl-x/releases/tag/v1.2.0):

**macOS (Apple Silicon):**
```bash
curl -L https://github.com/grauwen/utl-x/releases/download/v1.2.0/utlx-macos-arm64.bin -o utlx
chmod +x utlx
...
**Linux (x64):**
```bash
curl -L https://github.com/grauwen/utl-x/releases/download/v1.2.0/utlx-linux-x64.bin -o utlx
chmod +x utlx
sudo mv utlx /usr/local/bin/
```

**Windows:** Download `utlx-windows-x64.exe` from the [releases page](https://github.com/grauwen/utl-x/releases/tag/v1.2.0).
...
The wrapper scripts (`utlx`, `utlx.bat`, `utlx.ps1`) automatically locate and run the compiled JAR file at `modules/cli/build/libs/cli-1.2.0.jar`.
